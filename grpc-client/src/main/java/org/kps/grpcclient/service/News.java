...
public record News(String id, String name, int pageCount, String rating, String authorId) {

    private static List<News> NEWS = new ArrayList<>();

    static {
        NEWS.add(new News("news-1", "Effective Java", 416, Rating.FIVE_STAR.name(), "author-1"));
        NEWS.add(new News("news-2", "Hitchhiker's Guide to the Galaxy", 208, Rating.THREE_STAR.name(), "author-2"));
        NEWS.add(new News("news-3", "Down Under", 436, Rating.FOUR_STAR.name(), "author-3"));
    }
    ...
    public static News updateNews(String id, String name, int page, Rating rating, String authorId) {
        News updatedNews = new News(id, name, page, rating.name(), authorId);
